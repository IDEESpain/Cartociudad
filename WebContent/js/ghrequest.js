...
GHRequest.prototype.createURL = function(demoUrl) {    
    return this.createPath(this.host + "/geocoder/api/" + demoUrl); // + "&type=jsonp");
}
...
GHRequest.prototype.doRequest = function(url, callback) {   
    ...
    $.ajax({
        ...
        "success": function(json) {
            if(tmpEncodedPolyline && json.geom) { //route) {
                // convert encoded polyline stuff to normal json
                if (json.geom) {
                    var tmpArray = decodePath(json.geom, true);
                    json.route = {}; // parche
//                    json.info = {};
                    json.errors = null;
//                    json.info.routeFound = true;
...
GHRequest.prototype.getInfo = function(success, error) {
    var url = this.host + "/geocoder/api/info";
    console.log(url);    
    return $.ajax({
        "url": url,
        "success": success,
